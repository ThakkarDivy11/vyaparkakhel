...
export default function VsComputerSetup() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const totalPlayerCount = botCount + 1;

      // 1. Create the game with settings mode = vs_computer
      const res = await fetch(`${API_URL}/games`, {
        ...
        body: JSON.stringify({
          settings: { maxPlayers: totalPlayerCount, mode: 'vs_computer', boardTheme },
          playerNames: playerNames.slice(0, totalPlayerCount),
        }),
      });
      ...
      // 2. Start the game immediately since it features local bots
  ...
  const pct = ((botCount - MIN_BOTS) / (MAX_BOTS - MIN_BOTS)) * 100;
      ...
      <main className="h-screen max-h-screen w-full max-w-xl lg:max-w-2xl mx-auto pt-24 sm:pt-28 pb-3 px-4 sm:px-6 flex flex-col justify-between z-10 overflow-hidden">
        ...
        <div className="w-full flex flex-col items-center text-center max-w-md mx-auto mb-4 shrink-0">
          <div className="flex items-center gap-3 mb-2 justify-center">
            <Cpu size={28} className="text-[#ffd54f] drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" strokeWidth={2.25} />
          ...
          <p className="text-white/85 text-xs text-center leading-relaxed font-medium">
            Play against computer-controlled bot opponents.
          </p>
        </div>

        {/* Slider for bot count */}
        <div className="px-2 mb-4 shrink-0 max-w-md mx-auto w-full">
          <h3 className="text-xs font-bold text-white/70 mb-2 px-1">Number of Bots</h3>
          <div className="relative h-12 mb-2">
            <div
              className="absolute -translate-x-1/2 transition-[left] duration-150 ease-out"
              style={{ left: `${pct}%` }}
            >
              <div className="w-10 h-10 rounded-full bg-portage-600 text-white flex items-center justify-center font-extrabold text-lg shadow-[0_2px_4px_rgba(0,0,0,0.4)] tabular-nums">
                {botCount}
              </div>
            </div>
          </div>
          ...
          <input
            ...
            className="w-full h-2 bg-surface-2 border border-border rounded-full appearance-none cursor-pointer outline-none
              ...
              [&::-webkit-slider-thumb]:bg-portage-600
              ...
              [&::-moz-range-thumb]:bg-portage-600
              [&::-moz-range-thumb]:border-2
              [&::-moz-range-thumb]:border-white
              [&::-moz-range-thumb]:cursor-pointer"
          />
          <div className="flex justify-between mt-3 px-0.5 text-xs text-white/50 tabular-nums">
            {Array.from({ length: MAX_BOTS - MIN_BOTS + 1 }).map((_, i) => {
              const n = MIN_BOTS + i;
              return <span key={n} className={n === botCount ? 'text-white font-bold' : ''}>{n}</span>;
            })}
          </div>
        </div>

        {/* Player & Bot Name Inputs */}
        <div className="flex flex-col gap-2 mb-4 min-h-0 flex-1 overflow-hidden max-w-md mx-auto w-full">
          <h3 className="text-xs font-bold text-white/70 mb-1 px-1 shrink-0">Configure Names</h3>
          
          <div className="overflow-y-auto pr-1 flex flex-col gap-2 scrollbar-none flex-1 max-h-[160px] min-h-0">
            {/* Human player */}
            <div className="flex items-center gap-3 bg-surface-2 border border-border rounded-xl p-2.5 shrink-0">
              <div className="w-8 h-8 rounded-full bg-portage-100 text-portage-700 flex items-center justify-center font-bold text-sm shrink-0">
                You
              </div>
              <input
                type="text"
            ...
            {Array.from({ length: botCount }).map((_, i) => (
              <div key={i + 1} className="flex items-center gap-3 bg-surface-2 border border-border rounded-xl p-2.5 shrink-0">
                <div className="w-8 h-8 rounded-full bg-alabaster-100 text-alabaster-700 flex items-center justify-center font-bold text-sm shrink-0">
                  B{i + 1}
                </div>
                <input
                  type="text"
        ...
        <div className="bg-surface/10 border border-white/10 rounded-2xl p-3 mb-4 shrink-0 max-w-md mx-auto w-full">
